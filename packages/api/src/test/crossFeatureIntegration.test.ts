...
import {
  ...
  approveTask,
  completeTask,
} from "../services/tasks/task-lifecycle.js";
import {
  createSprint,
  startSprint,
  completeSprint,
...
function setupTeamWithUsers(habId: string, userNames: string[]) {
  ...
  db.insert(organizations)
    .values({ id: orgId, name: `Org-${ts}`, slug: `org-${ts}` })
    .run();

  const teamId = `team-cross-${ts}`;
  db.insert(teams)
    .values({ id: teamId, organizationId: orgId, name: `Team-${ts}`, slug: `team-${ts}` })
    .run();
  ...
  for (const name of userNames) {
    ...
    db.insert(users)
      .values({
        id: uid,
        username: `${name}-${ts}-${userIds.length}`,
        passwordHash: "hash",
        displayName: name,
      })
      .run();
...
function createTaskWithMission(title = "Test Task") {
  const mission = missionRepo.createMission({
    habitatId,
    columnId,
    title: `Mission for ${title}`,
    createdBy: "test",
  });
  return {
    mission,
    task: taskRepo.createTask({ missionId: mission.id, title, createdBy: "test" }),
  };
}
...
describe("Cross-feature: Sprint carry-over with review rules active", () => {
  ...
  it("review rules do not interfere with sprint state transitions", () => {
    ...
    const { mission, task } = createTaskWithMission("Sprint Mission");
    addMissionToSprint(sprint.id, mission.id);

    startSprint(sprint.id);
    expect(sprintRepo.getById(sprint.id)?.status).toBe("active");

    // Exercise the review rule to verify it's genuinely cross-cutting:
    // a submitted task in an active sprint should get reviewers assigned.
    claimStartSubmit(task.id);
    const reviewers = taskReviewerRepo.getByTaskId(task.id);
    expect(reviewers.length).toBeGreaterThanOrEqual(1);
...
describe("Cross-feature: Multiple sprints with review rules", () => {
  it("completes sprint 1, creates sprint 2, and assigns reviewers in sprint 2", () => {
    ...
    const sprint2 = createSprint(
      habitatId,
      { name: "Sprint 2", startDate: "2026-06-15", endDate: "2026-06-28" },
      "user-1",
    );
