...
describe("syncService", () => {
  ...
  it("emits integration-sourced mission events for create and update side effects", async () => {
    const adapter1 = makeFakeAdapter([makeIssue()]);
    const first = await syncConnection(connectionId, "manual", adapter1);
    const mission = missionRepo.getMissionsByHabitatId(habitatId).missions[0];

    const adapter2 = makeFakeAdapter([makeIssue({ title: "Updated Title" })]);
    const second = await syncConnection(connectionId, "manual", adapter2);

    const events = missionEventRepo.getMissionEventsByMissionId(mission.id, 10).events;

    expect(events).toHaveLength(2);
    expect(events.map((event) => event.action).toSorted()).toEqual(["created", "updated"]);
    ...
    expect(
      events.map((event) => (event.metadata as any).audit.integrationSyncRunId).toSorted(),
    ).toEqual([first.syncRunId, second.syncRunId].toSorted());
  });
  ...
  it("closed issue with no tasks marks mission done", async () => {
    ...
    const events = missionEventRepo.getMissionEventsByMissionId(mission.id, 10).events;
    expect(events.some((event) => event.action === "status_changed")).toBe(true);
  });
...
describe("syncService — intake candidate path", () => {
  ...
  it("does not auto-ignore candidate that is already promoted", () => {
    ...
    candidateRepo.update(candidates[0].id, {
