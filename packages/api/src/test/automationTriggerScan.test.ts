...
describe("automationEventService", () => {
  ...
  it("returns empty result for non-allowlisted event", () => {
    const result = ingestEvent("hab-1", { type: "unknown.event", data: {} });
    expect(result.eventType).toBe("unknown.event");
    expect(result.matched).toBe(0);
    expect(result.skipped).toBe(0);
  });

  it("returns empty when no matching enabled rules exist", () => {
    const habitat = setupHabitat();
    const result = ingestEvent(habitat.id, { type: "task.rejected", data: {} });
    expect(result.eventType).toBe("task.rejected");
    expect(result.matched).toBe(0);
    expect(result.skipped).toBe(0);
  });

  it("task.rejected event triggers a matching rule", () => {
    const habitat = setupHabitat();
    createEnabledRule(habitat.id, "task.rejected");

    const result = ingestEvent(habitat.id, {
      type: "task.rejected",
  ...
  it("duplicate event within cooldown records skipped run", () => {
    const habitat = setupHabitat();
    createEnabledRule(habitat.id, "task.rejected", { cooldownSeconds: 3600 });

    const data = { taskId: "task-1", eventId: "evt-1" };

    const first = ingestEvent(habitat.id, { type: "task.rejected", data });
    expect(first.matched).toBeGreaterThanOrEqual(1);

    const second = ingestEvent(habitat.id, { type: "task.rejected", data });
    expect(second.skipped).toBeGreaterThanOrEqual(1);
  });

  it("skips when hourly cap is exceeded", () => {
    const habitat = setupHabitat();
    ...
    const second = ingestEvent(habitat.id, {
      type: "task.rejected",
  ...
  it("automation event with self-loop provenance is skipped", () => {
    const habitat = setupHabitat();
  ...
  it("ingestion is non-throwing on bad data", () => {
    const habitat = setupHabitat();
    createEnabledRule(habitat.id, "task.rejected");

    const result = ingestEvent(habitat.id, { type: "task.rejected", data: null as any });
    expect(result.errors).toBeDefined();
  });

  it("event with task target resolves targetType correctly", () => {
    const habitat = setupHabitat();
  ...
  it("mission event triggers matching rule", () => {
    const habitat = setupHabitat();
  ...
  it("pulse event triggers matching rule", () => {
    const habitat = setupHabitat();
  ...
  it("sprint event triggers matching rule", () => {
    const habitat = setupHabitat();
...
describe("automationScanService", () => {
  ...
  it("returns empty when no rules match scan types", () => {
    const habitat = setupHabitat();
  ...
  it("mission_blocked scan triggers rule", () => {
    const habitat = setupHabitat();
  ...
  it("agent_silent scan triggers rule", () => {
    const habitat = setupHabitat();
  ...
  it("evidence_gap_open scan triggers rule", () => {
    const habitat = setupHabitat();
  ...
  it("sprint_ending scan runs when sprint exists", () => {
    const habitat = setupHabitat();
  ...
  it("scan events use deterministic trigger_event_id", () => {
    const habitat = setupHabitat();
    createEnabledRule(habitat.id, "mission_blocked");
    runAllScans();
  ...
  it("disabled rules are not triggered by scans", () => {
    const habitat = setupHabitat();
