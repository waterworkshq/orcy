...
vi.mock("../services/auditQueryService.js", () => ({
    ...
    events: [
      {
        ...
        metadata: {},
        completeness: { status: "legacy_partial", caveats: [] },
      },
    ],
    warnings: [],
    completenessSummary: {
      totalEvents: 1,
      byStatus: { complete: 0, legacy_partial: 1, source_unavailable: 0 },
      caveats: [],
    },
  })),
  summarizeAuditCompleteness: vi.fn(() => ({
    totalEvents: 2,
  ...
  describe("services/gitWorktreeService.ts - cleanup catch blocks", () => {
    ...
    it("logs warning when fallback worktree creation fails", () => {
      ...
      vi.mocked(execFileSync)
        ...
        .mockImplementationOnce(() => {
          throw new Error("detached add failed");
        })
        .mockImplementationOnce(() => {
