...
vi.mock("uuid", () => ({
  v4: vi.fn(() => "mock-uuid-mission"),
}));

import { createMentions, getMentionsByCommentIds } from "../repositories/featureCommentMention.js";

describe("featureCommentMention repository", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    _mentionsStore = {};
    _selectAll = vi.fn(() => []);
  ...
  describe("createMentions", () => {
    it("creates mentions and returns them with id and createdAt", () => {
      ...
      expect(result[0].id).toBe("mock-uuid-mission");
