...
describe("Route Inventory", () => {
  ...
  beforeAll(async () => {
    ...
    app.addHook("onRoute", (routeOptions) => {
      ...
      // may be prepended after this observer runs, so the declaration itself
  ...
  describe("/api/v1 routes", () => {
    ...
    it("reports unauthenticated non-public routes", () => {
      ...
      expect(
        unexpected,
        `Unauthenticated /api/v1 routes without auth: ${unexpected.join(", ")}`,
      ).toEqual([]);
    });
  ...
  describe("/api routes (deprecated prefix)", () => {
    ...
    it("reports unauthenticated non-public routes", () => {
      ...
      expect(
        unexpected,
        `Unauthenticated /api routes without auth: ${unexpected.join(", ")}`,
      ).toEqual([]);
    });
