...
export async function habitatRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.withTypeProvider<ZodTypeProvider>().get(
    "/habitats",
    {
      schema: { querystring: z.object({ name: z.string().optional() }) },
  ...
  fastify
    ...
    .post(
      "/habitats",
      { schema: { body: createHabitatSchema }, config: { authPolicy: "human" } },
      async (request, reply) => {
  ...
  fastify
    ...
    .post(
      "/habitats/agent",
      { schema: { body: createHabitatSchema }, config: { authPolicy: "agent" } },
      async (request, reply) => {
        const { habitat, columns } = habitatService.createHabitat({
          ...request.body,
          defaultColumns: request.body.defaultColumns ?? true,
