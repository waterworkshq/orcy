...
export async function habitatAnalyticsRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  /** GET /habitats/:habitatId/cumulative-flow - Get cumulative-flow chart data. Auth: agentOrHumanAuth + board access. Query: ?days=30 */
  fastify.withTypeProvider<ZodTypeProvider>().get(
    "/habitats/:habitatId/cumulative-flow",
    {
      schema: {
        params: habitatIdParamsSchema,
    ...
    async (request, _reply) => {
      ...
      return cumulativeFlowService.getCumulativeFlow(request.params.habitatId, days);
    },
  );

  /** GET /habitats/:habitatId/bottlenecks - Get bottleneck findings. Auth: agentOrHumanAuth + board access. Query: ?days=30 */
  fastify.withTypeProvider<ZodTypeProvider>().get(
    "/habitats/:habitatId/bottlenecks",
    {
      schema: {
        params: habitatIdParamsSchema,
        querystring: z.object({ days: z.coerce.number().int().min(7).max(90).optional() }),
