...
import { publishHabitatUpdate } from "../services/habitatService.js";
...
const nonRecursiveConditionSchema = z.discriminatedUnion("type", [
  ...
  z.object({
    type: z.literal("dependency_count"),
    greaterThan: z.number(),
    direction: z.union([z.literal("blocking"), z.literal("blocked_by")]),
  }),
  ...
  fastify.put(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (!parsed.success) {
        return reply
          .status(400)
          .send({ error: "Invalid rules payload", details: parsed.error.flatten() });
      }
      ...
      const current =
        habitat.prioritizationSettings ?? prioritizationService.getDefaultPrioritizationSettings();
      ...
      const updatedHabitat = habitatRepo.updateHabitat(params.habitatId, {
        prioritizationSettings: updated,
      });
      if (updatedHabitat) {
        publishHabitatUpdate(params.habitatId, updatedHabitat);
      }
