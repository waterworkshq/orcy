...
export function listByHabitat(habitatId: string): IntegrationConnection[] {
  ...
  return db
    .select()
    .from(integrationConnections)
    .where(eq(integrationConnections.habitatId, habitatId))
...
export function listEnabledByProvider(provider: IntegrationProvider): IntegrationConnection[] {
  ...
  return db
    ...
    .from(integrationConnections)
    .where(
      and(eq(integrationConnections.provider, provider), eq(integrationConnections.enabled, true)),
    )
    .all() as IntegrationConnection[];
}

export function listEnabledByProviderAndRepo(
  provider: IntegrationProvider,
  owner: string,
  repo: string,
): IntegrationConnection[] {
...
export function update(
  ...
  if (input.externalAccountName !== undefined)
    values.externalAccountName = input.externalAccountName;

  try {
    db.update(integrationConnections).set(values).where(eq(integrationConnections.id, id)).run();
  } catch (err) {
    throw repositoryUpdateError("integrationConnection", err as Error, id);
  }
  return getById(id);
}
