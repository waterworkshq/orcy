...
import { pipelineEvents } from '../db/schema.js';
...
export function findByProviderAndRunId(provider: 'github' | 'gitlab', repo: string, runId: string): PipelineEvent | null {
  const db = getDb();
  const rows = db.select().from(pipelineEvents).where(
    and(eq(pipelineEvents.provider, provider), eq(pipelineEvents.repo, repo), eq(pipelineEvents.runId, runId))
  ).all();
  return rows.length > 0 ? rows[0] as PipelineEvent : null;
}

export function updatePipelineEvent(id: string, updates: {
  status?: PipelineEvent['status'];
  commitSha?: string;
}): PipelineEvent | null {
  const db = getDb();
