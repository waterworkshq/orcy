...
export function getMessagesByAgent(
  ...
  const messages = db.select().from(agentMessages)
    .where(where)
...
export function getUnreadCount(agentId: string): number {
  const db = getDb();
  const rows = db.select({ count: count() }).from(agentMessages).where(
    and(eq(agentMessages.toAgentId, agentId), isNull(agentMessages.readAt))
  ).all();
...
export function markAsRead(messageId: string): AgentMessage | null {
  ...
  db.update(agentMessages).set({ readAt: now }).where(
    and(eq(agentMessages.id, messageId), isNull(agentMessages.readAt))
  ).run();
...
export function markAllAsRead(agentId: string): number {
  ...
  const totalRows = db.select({ count: count() }).from(agentMessages).where(
    and(eq(agentMessages.toAgentId, agentId), isNull(agentMessages.readAt))
  ).all();
