...
import type { ExternalIntakeCandidate, ExternalIntakeReviewStatus, IntegrationProvider } from '@orcy/shared';
...
export function create(input: {
  ...
  db.insert(externalIntakeCandidates).values({
    id,
...
export function getById(id: string): ExternalIntakeCandidate | null {
  const db = getDb();
  return db.select().from(externalIntakeCandidates).where(eq(externalIntakeCandidates.id, id)).get() as ExternalIntakeCandidate | null;
}

export function findByConnectionAndExternalId(connectionId: string, externalId: string): ExternalIntakeCandidate | null {
  const db = getDb();
  return db.select().from(externalIntakeCandidates)
    .where(and(eq(externalIntakeCandidates.connectionId, connectionId), eq(externalIntakeCandidates.externalId, externalId)))
    .get() as ExternalIntakeCandidate | null;
}

export function listByHabitat(habitatId: string, filters?: { reviewStatus?: ExternalIntakeReviewStatus; provider?: IntegrationProvider }): ExternalIntakeCandidate[] {
  const db = getDb();
...
export function update(id: string, input: {
  ...
  if (input.recommendedMissionTitle !== undefined) values.recommendedMissionTitle = input.recommendedMissionTitle;
  if (input.recommendedMissionDescription !== undefined) values.recommendedMissionDescription = input.recommendedMissionDescription;
  ...
  db.update(externalIntakeCandidates).set(values).where(eq(externalIntakeCandidates.id, id)).run();
  return getById(id);
}
