...
export function findByProviderAndNumber(provider: 'github' | 'gitlab', repo: string, prNumber: number): PullRequest | null {
  const db = getDb();
  const rows = db.select().from(pullRequests).where(
    and(eq(pullRequests.provider, provider), eq(pullRequests.repo, repo), eq(pullRequests.prNumber, prNumber))
  ).all();
  return rows.length > 0 ? rows[0] as PullRequest : null;
}

export function updatePullRequest(id: string, updates: {
  prTitle?: string;
  state?: 'open' | 'merged' | 'closed';
  reviewStatus?: 'pending' | 'approved' | 'changes_requested';
}): PullRequest | null {
