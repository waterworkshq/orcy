...
export function createOrganization(input: { name: string; slug: string }): Organization {
  ...
  try {
    ...
    throw repositoryCreateError("organization", err as Error, id);
  }

  return assertFound(getOrganizationById(id), "organization", id);
}
...
export function deleteOrganization(id: string): void {
  ...
  try {
    ...
    throw repositoryDeleteError("organization", err as Error, id);
  }
}
