...
export function getCapacityReport(boardId: string): CapacityReport | null {
  ...
  const agentCapacities: AgentCapacity[] = agents.map(agent => {
    const activeRow = db.select({ count: sql<number>`count(*)` })
      .from(tasks)
      .where(and(eq(tasks.assignedAgentId, agent.id), inArray(tasks.status, ['claimed', 'in_progress'])))
      .get();
