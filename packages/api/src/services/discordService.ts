...
/** Parsed subcommand and arguments extracted from a Discord `/orcy` interaction. */
export interface ParsedCommand {
  action: string;
  args: string[];
}

/** Extracts the subcommand and positional args from a Discord `/orcy` interaction payload, falling back to a `help` action when the structure is unrecognized. */
export function parseDiscordCommand(data: {
  name?: string;
  options?: Array<{
    name: string;
    value: string;
...
/** Builds a Discord webhook payload (content plus embed) for a single task lifecycle event. */
...
/** Builds a Discord embed listing up to ten pending tasks for the `/orcy list` response. */
export function formatDiscordTaskList(
...
/** Builds a Discord embed with a single task's details for the `/orcy info` response. */
export function formatDiscordTaskInfo(task: {
  id: string;
  title: string;
  status: string;
  priority: string;
...
/** Builds the Discord embed listing the available `/orcy` commands for the help response. */
...
/** Builds a Discord embed for a generic success or failure response message. */
