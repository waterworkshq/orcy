...
export function getApprovalStatus(taskId: string): ApprovalStatus {
  ...
  const qualityStatus = qualityReport.canApprove ? 'complete' : (qualityCompleted > 0 ? 'incomplete' : 'not_started');
  ...
  const db = getDb();
  const depResult = db.select({ count: count() })
  ...
  const timeStatus = task.actualMinutes !== null && task.actualMinutes > 0 ? 'complete' : 'not_started';
