...
export function delegateTask(
  ...
  if (!task.assignedAgentId) {
    return { success: false, reason: 'not_assigned', message: 'Task must be claimed before delegation' };
  }

  if (task.assignedAgentId !== fromAgentId) {
    return { success: false, reason: 'not_owner', message: 'Only the assigned agent can delegate this task' };
  }

  if (fromAgentId === toAgentId) {
    return { success: false, reason: 'self_delegation', message: 'Cannot delegate a task to yourself' };
  }

  if (task.status !== 'claimed' && task.status !== 'in_progress') {
    return { success: false, reason: 'invalid_status', message: `Task in status '${task.status}' cannot be delegated. Must be claimed or in_progress.` };
  }
  ...
  if (task.requiredCapabilities && task.requiredCapabilities.length > 0) {
    ...
    if (missing.length > 0) {
      return { success: false, reason: 'capability_mismatch', message: `Target agent lacks required capabilities: ${missing.join(', ')}` };
    }
  }

  if (task.requiredDomain && targetAgent.domain !== task.requiredDomain && targetAgent.domain !== 'fullstack') {
    return { success: false, reason: 'domain_mismatch', message: `Target agent domain '${targetAgent.domain}' does not match required domain '${task.requiredDomain}'` };
  }
...
export function claimDelegatedTask(taskId: string, agentId: string): { success: true; task: Task } | { success: false; reason: string; message?: string } {
