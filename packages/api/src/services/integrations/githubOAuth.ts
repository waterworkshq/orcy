...
export async function startGitHubDeviceFlow(): Promise<GitHubDeviceFlowStart> {
  ...
  if (!res.ok) {
    ...
    throw new Error(body.error_description || body.error || `Device flow start failed (HTTP ${res.status})`);
  }

  return res.json() as Promise<GitHubDeviceFlowStart>;
}

export async function pollGitHubDeviceFlow(deviceCode: string): Promise<GitHubDeviceFlowPollResult> {
  const clientId = getClientId();
  ...
  if (!res.ok) {
    ...
    throw new Error(body.error_description || body.error || `Token polling failed (HTTP ${res.status})`);
  }
