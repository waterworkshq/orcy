...
export function normalizeGitHubIssue(owner: string, repo: string, issue: GitHubIssue): ExternalIssue {
  return {
    provider: 'github',
...
export const githubAdapter: IssueProviderAdapter = {
  ...
  async listIssues(connection: IntegrationConnection): Promise<ExternalIssue[]> {
    ...
    let url: string | null = `${GITHUB_API_BASE}/repos/${owner}/${repo}/issues?state=open&per_page=100`;
...
export async function createGitHubWebhook(connection: IntegrationConnection, webhookUrl: string): Promise<{ externalId: string; warning?: string }> {
  const token = connection.accessToken;
  if (!token) throw new Error('GitHub connection has no access token');
  ...
  try {
    ...
    if (response.status === 403) {
      return {
        externalId: '',
        warning: 'Insufficient permissions to create webhook. Manual sync still works. Required scope: admin:repo_hook',
      };
    }
