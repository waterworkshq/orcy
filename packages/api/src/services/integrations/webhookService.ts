...
function normalizeWebhookIssue(owner: string, repo: string, issue: GitHubWebhookIssue): ExternalIssue {
  return {
    provider: 'github',
...
export function handleGitHubIssueWebhook(
  ...
  for (const connection of connections) {
    ...
    if (signature && !verifyGitHubHmac(rawBody, signature, connection.webhookSecret)) {
      logger.warn({ connectionId: connection.id }, 'GitHub issue webhook signature verification failed');
      continue;
    }
    ...
    try {
      ...
      logger.warn({ err, connectionId: connection.id, externalId: normalizedIssue.externalId }, 'Failed to process GitHub issue webhook');
    }
  }
