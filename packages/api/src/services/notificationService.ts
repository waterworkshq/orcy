...
function getPreferenceValue(prefs: NotificationPreferences, field: string): boolean | undefined {
  switch (field) {
    ...
    case "taskSubmitted":
      return prefs.taskSubmitted;
    case "taskApproved":
      return prefs.taskApproved;
    case "taskRejected":
      return prefs.taskRejected;
    case "taskOverdue":
      return prefs.taskOverdue;
    case "taskMentioned":
      return prefs.taskMentioned;
    case "taskWatching":
      return prefs.taskWatching;
    case "taskPriorityChanged":
      return prefs.taskPriorityChanged;
    case "taskReviewAssigned":
      return prefs.taskReviewAssigned;
    default:
      return undefined;
  }
}
...
async function sendIfEnabled(
  ...
  habitatId: string,
  eventType: EventType,
  buildEmail: () => emailService.EmailPayload,
): Promise<void> {
  ...
  const enabled =
    getPreferenceValue(habitatPrefs, prefField) ??
    getPreferenceValue(globalPrefs, prefField) ??
    false;
  if (!enabled) return;

  const email = userRepo.getUserEmail(userId);
  if (!email) return;
...
export async function processEvent(
  ...
  const actorName = data.actorId ? userRepo.getActorName(data.actorId) : "System";
  ...
  switch (eventType) {
    ...
    case "task.submitted": {
      const adminRows = userRepo.getAdmins();
    ...
    case "task.rejected": {
      if (task.assignedAgentId) {
        queue.push({
          ...
          buildEmail: () =>
            emailService.taskRejectedTemplate(
              taskTitle,
              habitatName,
              actorName,
    ...
    case "task.overdue": {
      if (task.assignedAgentId) {
        queue.push({
          ...
          buildEmail: () =>
            emailService.taskOverdueTemplate(
              taskTitle,
              habitatName,
              mission?.dueAt ?? mission?.slaDeadlineAt ?? "Unknown",
            ),
        });
      }
      const adminRows2 = userRepo.getAdmins();
    ...
    case "comment.mentioned": {
      if (data.mentionedUserId) {
        queue.push({
          ...
          buildEmail: () =>
            emailService.commentMentionedTemplate(
              taskTitle,
              habitatName,
              data.mentionedByName ?? actorName,
              data.commentContent ?? "",
            ),
        });
      }
    ...
    case "task.priority_changed": {
      ...
      if (task.assignedAgentId) {
        queue.push({
          ...
          buildEmail: () =>
            emailService.priorityChangedTemplate(
              taskTitle,
              habitatName,
              data.oldPriority ?? "medium",
              data.newPriority ?? "medium",
            ),
        });
      }
      // Also notify admins
      const adminRows3 = userRepo.getAdmins();
      for (const row of adminRows3) {
        if (row.id !== task.assignedAgentId) {
          // avoid duplicate
          queue.push({
            userId: row.id,
            eventType: "task.priority_changed",
