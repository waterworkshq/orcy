...
export const pulses = sqliteTable(
  "pulses",
  {
    ...
    scope: text("scope", { enum: ["mission", "habitat"] })
      .notNull()
      .default("mission"),
  ...
  (table) => [
    ...
    index("idx_pulses_habitat").on(table.habitatId),
    index("idx_pulses_scope").on(table.scope),
    index("idx_pulses_habitat_scope").on(table.habitatId, table.scope),
...
export const pulseCursors = sqliteTable(
  "pulse_cursors",
  {
  ...
  (table) => [primaryKey({ columns: [table.scopeKey, table.readerType, table.readerId] })],
);
