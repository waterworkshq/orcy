...
export const api = {
  ...
  integrations: {
    ...
    completeJiraOAuth: (habitatId: string, data: { code: string; state: string; redirectPort: number }) =>
      request<{ integration: IntegrationConnectionView }>(
        `/habitats/${habitatId}/integrations/jira/oauth/complete`,
        { method: 'POST', body: JSON.stringify(data) }
      ),
    createJiraApiKey: (habitatId: string, data: {
      name: string;
      email: string;
      token: string;
      siteUrl: string;
      ...
      request<{ integration: IntegrationConnectionView }>(`/habitats/${habitatId}/integrations/jira/api-key`, {
        method: 'POST',
        body: JSON.stringify(data),
      }),
    ...
    completeLinearOAuth: (habitatId: string, data: { code: string; state: string; redirectPort: number }) =>
      request<{ integration: IntegrationConnectionView; teams: Array<{ id: string; name: string; key: string }> }>(
        `/habitats/${habitatId}/integrations/linear/oauth/complete`,
        { method: 'POST', body: JSON.stringify(data) }
      ),
    createLinearApiKey: (habitatId: string, data: {
      name: string;
      token: string;
      teamId: string;
      autoImport?: boolean;
      ...
      request<{ integration: IntegrationConnectionView }>(`/habitats/${habitatId}/integrations/linear/api-key`, {
        method: 'POST',
        body: JSON.stringify(data),
      }),
    listIntakeCandidates: (habitatId: string, filters?: { reviewStatus?: string; provider?: string }) => {
      const params = new URLSearchParams();
    ...
    ignoreCandidate: (candidateId: string) =>
      request<{ candidate: ExternalIntakeCandidate }>(`/intake-candidates/${candidateId}/ignore`, { method: 'POST' }),
