...
export const api = {
  ...
  /** Workflow management — per-mission DAG visualization, gate unblock, and detach. */
  workflows: {
  ...
  wiki: {
    ...
    getCadence: (habitatId: string) =>
      request<{ cadence: WikiCadence }>(`/habitats/${habitatId}/wiki/cadence`).then(
        (r) => r.cadence,
      ),
    setCadence: (
      habitatId: string,
  ...
  automation: {
    ...
    run: (ruleId: string) =>
      request<unknown>(`/automation-rules/${ruleId}/run`, { method: "POST" }),
  ...
  remoteAccess: {
    ...
    updateProvider: (habitatId: string, providerId: string, body: Record<string, unknown>) =>
      request<{ provider: unknown }>(
        `/habitats/${habitatId}/remote-access/providers/${providerId}`,
  ...
  /**
   * Plugin enrollment management (ADR-0013, ADR-0016). Enrollments bind a loaded
   * plugin contribution to a habitat; runs are the execution audit trail.
   */
  plugins: {
    ...
    updateEnrollment: (
      ...
      request<{ enrollment: unknown }>(
        `/habitats/${habitatId}/plugins/enrollments/${enrollmentId}`,
    ...
    listRuns: (
      ...
      return request<{ runs: unknown[]; total: number }>(
        `/habitats/${habitatId}/plugins/runs${qs}`,
      );
    },
