      ...
      <ReactMarkdown
        ...
        components={{
          ...
          code: ({ inline, children, ...props }: any) =>
            inline ? (
              <code className="rounded bg-secondary px-1 py-0.5 text-xs font-mono" {...props}>
                {children}
              </code>
            ) : (
              <code
                className="block rounded bg-secondary p-2 text-xs font-mono overflow-x-auto"
                {...props}
              >
                {children}
              </code>
          ...
          pre: ({ children }) => (
            <pre className="rounded bg-secondary p-3 overflow-x-auto">{children}</pre>
          ),
          ...
          th: ({ children }) => (
            <th className="border border-border px-2 py-1 text-left font-semibold bg-secondary">
              {children}
            </th>
          ),
