...
const navTabs = [
  { label: 'Echo Base', href: '/', match: (path: string) => path === '/' || path.startsWith('/boards') || path.startsWith('/features'), description: 'Workspace board' },
  { label: 'Orcy Pod', href: '/agents', match: (path: string) => path.startsWith('/agents'), description: 'Agent management' },
  { label: 'Wake', href: '/activity', match: (path: string) => path.startsWith('/activity'), description: 'Activity log' },
  { label: 'Pod Base', href: '/dashboard', match: (path: string) => path.startsWith('/dashboard'), description: 'Analytics dashboard' },
];
...
export const TopAppBar = React.memo(function TopAppBar() {
  ...
  return (
    <header className="glass-panel ghost-border-b flex h-16 shrink-0 items-center justify-between px-4 md:px-6" data-testid="top-app-bar">
      <div className="flex min-w-0 items-center gap-4 md:gap-6">
        ...
        <nav className="hidden items-center gap-1 font-headline text-xs font-bold uppercase tracking-wide md:flex" aria-label="Primary navigation">
          {navTabs.map((tab) => {
      ...
      <div className="flex items-center gap-2 md:gap-3">
        <div className="hidden items-center gap-2 rounded-full bg-surface-container-high px-3 py-1.5 lg:flex" data-testid="fleet-pulse">
          <span className="text-[10px] font-medium uppercase tracking-wider text-on-surface-variant">Fleet Pulse</span>
          {agents.length > 0 ? agents.slice(0, 4).map((agent) => {
            const { color, label, pulse } = agentStatusConfig[agent.status] ?? agentStatusConfig.idle;
            return (
              <div key={agent.id} className="flex items-center gap-1.5" title={`${agent.name}: ${label}`}>
                <div className={`h-2 w-2 rounded-full ${color}${pulse ? ' animate-pulse' : ''}`} />
          ...
          {agents.length > 4 && (
            <span className="text-[10px] font-medium text-on-surface-variant">+{agents.length - 4}</span>
          )}
        </div>
        ...
        <div className="absolute right-0 top-full mt-2 z-50" data-testid="user-menu-dropdown">
          <div className="glass-card w-56 rounded-xl shadow-xl border border-outline-variant/15 overflow-hidden">
            <div className="px-3 py-2.5 border-b border-outline-variant/15">
              <p className="text-sm font-medium text-on-surface truncate" data-testid="user-menu-username">
                {username}
              </p>
            </div>
