...
vi.mock("./TaskDistribution.js", () => ({
  TaskDistribution: () => <div data-testid="task-distribution">TaskDistribution</div>,
}));
...
vi.mock("./FlowAnalyticsPanel.js", () => ({
  FlowAnalyticsPanel: ({ habitatId }: { habitatId: string }) => (
    <div data-testid="flow-analytics-panel">Flow:{habitatId}</div>
  ),
}));
...
describe("DashboardCharts", () => {
  ...
  it("renders CapacityChart when habitatId is provided", async () => {
    ...
    expect(screen.getByTestId("flow-analytics-panel")).toHaveTextContent("Flow:board-1");
  });
