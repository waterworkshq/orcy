...
const baseFeatures = [
  { id: 'f1', title: 'Feature 1', status: 'done', dependsOn: [] },
  { id: 'f2', title: 'Feature 2', status: 'in_progress', dependsOn: [] },
];
...
describe('StatsModal', () => {
  ...
  it('uses backdrop-blur-2xl for frosted glass effect', async () => {
    ...
    const withBlur = Array.from(allElements).filter(
      (el) => el.className && typeof el.className === 'string' && el.className.includes('backdrop-blur-2xl'),
    );
