...
export function ReviewRulesTab({ habitatId }: ReviewRulesTabProps) {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, body }: { id: string; body: ReviewRuleUpdateInput }) =>
      api.reviewRules.update(id, body),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.reviewRules.list(habitatId) });
  ...
  if (loading) {
    return (
      <div className="py-8 text-center text-sm text-muted-foreground">Loading review rules...</div>
    );
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Configure automatic reviewer assignment when tasks are submitted
          </p>
        </div>
        {editingId !== "new" && (
      ...
      {rules.length === 0 && editingId !== "new" && (
        <div className="rounded border border-border p-6 text-center">
          <p className="text-sm text-muted-foreground">
            No review rules configured. Tasks will be approved without review.
          </p>
        </div>
      )}
            ...
            <div className="flex items-center justify-between p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {rule.requiredReviews} review{rule.requiredReviews > 1 ? "s" : ""} via{" "}
                    {STRATEGY_OPTIONS.find((s) => s.value === rule.assignmentStrategy)?.label ??
                      rule.assignmentStrategy}
      ...
      <details className="border border-border rounded-md">
        ...
        <div className="px-3 pb-3 text-xs text-muted-foreground space-y-1">
          <p>
            When a task is submitted, review rules are matched in priority order. The first matching
            rule assigns reviewers.
          </p>
          <p>
            Leave match fields empty to match all tasks. Set multiple conditions to narrow the
            match.
          </p>
...
function RuleFormFields({
  ...
  return (
    <>
      <div>
        <label className="mb-1 block text-xs text-muted-foreground" htmlFor="rule-name">
          Rule Name
        </label>
        <input
          id="rule-name"
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="mb-1 block text-xs text-muted-foreground" htmlFor="rule-strategy">
            Assignment Strategy
          </label>
          <select
            id="rule-strategy"
            ...
            onChange={(e) =>
              setForm((f) => ({ ...f, assignmentStrategy: e.target.value as ReviewRuleStrategy }))
            }
            className="w-full rounded border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary"
          >
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="mb-1 block text-xs text-muted-foreground" htmlFor="rule-domain">
            Match Domain (optional)
          </label>
          <select
            id="rule-domain"
        ...
        <div>
          <label className="mb-1 block text-xs text-muted-foreground" htmlFor="rule-priority">
      ...
      <div>
        <label className="mb-1 block text-xs text-muted-foreground" htmlFor="rule-labels">
          Match Labels (comma-separated, optional)
        </label>
        <input
          id="rule-labels"
          type="text"
      ...
      <div className="space-y-2 pt-2 border-t border-border">
        <label className="flex items-center gap-2 cursor-pointer">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Don't allow the task submitter to review their own task
            </p>
          </div>
        </label>
      </div>
