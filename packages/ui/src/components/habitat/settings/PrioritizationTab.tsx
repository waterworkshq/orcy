...
import type { Habitat, PrioritizationSettings, PrioritizationRule, PrioritizationRuleCondition, PrioritizationRuleAction } from '../../../types/index.js';
...
type FieldSpec =
  ...
  | { kind: 'text'; key: string; label: string; placeholder?: string; toInput?: (v: any) => string; toState?: (v: string) => any }
  ...
  if (field.kind === 'number') {
    ...
    return (
      <div key={field.key}>
        ...
        <input type="number" min={field.min} value={value}
          onChange={e => onChange({ ...obj, [field.key]: parseInt(e.target.value, 10) || (field.fallback ?? 0) } as T)}
          className={INPUT_CLS} />
      </div>
    );
  }
    ...
    return (
      <div key={field.key}>
        ...
        <input type="text" value={value}
          onChange={e => onChange({ ...obj, [field.key]: field.toState ? field.toState(e.target.value) : e.target.value } as T)}
          placeholder={field.placeholder} className={INPUT_CLS} />
      </div>
    );
  }
    ...
    return (
      <div className="rounded bg-amber-500/10 border border-amber-500/30 p-2 space-y-2">
        ...
        <div>
          ...
          <select value="" onChange={e => onChange(getDefaultCondition(e.target.value))} className={INPUT_CLS}>
  ...
  return (
    <div className="space-y-2">
      <div>
        ...
        <select value={condition.type} onChange={e => onChange(getDefaultCondition(e.target.value))} className={INPUT_CLS}>
          {CONDITION_OPTIONS.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
        </select>
      </div>
      <div className={fieldsCls}>
        {cfg?.fields.map(f => renderField(condition, f, onChange))}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-2">
      <div>
        ...
        <select value={action.type} onChange={e => onChange(getDefaultAction(e.target.value))} className={INPUT_CLS}>
          {ACTION_OPTIONS.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
        </select>
      </div>
      <div className={fieldsCls}>
        {cfg?.fields.map(f => renderField(action, f, onChange))}
      </div>
    </div>
  );
}

function SortableRuleCard({ rule, expanded, onExpand, onUpdate, onDelete }: {
      ...
      {expanded ? (
        <div className="p-3 space-y-3">
          <div className="flex items-center gap-2">
            <button type="button" {...listeners} className="cursor-grab active:cursor-grabbing text-muted-foreground hover:text-foreground flex-shrink-0">
              <GripVertical className="h-4 w-4" />
            </button>
            <ToggleSwitch checked={rule.enabled} onChange={val => onUpdate({ ...rule, enabled: val })} />
            <input
              type="text"
              value={rule.name}
            ...
            />
            <button type="button" onClick={onExpand} className="text-xs text-muted-foreground hover:text-foreground whitespace-nowrap">Done</button>
            <button type="button" onClick={onDelete} className="text-destructive hover:text-destructive/80">
              <Trash2 className="h-4 w-4" />
            </button>
          </div>
          <div className="grid grid-cols-2 gap-3 pl-7">
            <ConditionEditor condition={rule.condition} onChange={c => onUpdate({ ...rule, condition: c })} />
        ...
        <div className="flex items-center gap-3 p-3">
          ...
          <button type="button" onClick={onExpand} className="text-xs text-muted-foreground hover:text-foreground whitespace-nowrap">Edit</button>
          <button type="button" onClick={onDelete} className="text-destructive hover:text-destructive/80">
            <Trash2 className="h-4 w-4" />
          </button>
        </div>
...
export const PrioritizationTab = forwardRef<PrioritizationTabHandle, PrioritizationTabProps>(function PrioritizationTab({
  ...
  }, [saveSettings, enabled, evaluateInterval, rules, fallbackToManual, advancedOpen, advancedText]);
        ...
        <div className="space-y-3">
          ...
          {rules.length === 0 ? (
            <div className="rounded border border-border p-4 text-center">
              <p className="text-sm text-muted-foreground">No rules configured. Add a rule to start auto-prioritizing tasks.</p>
            </div>
          ) : (
            ...
            {advancedOpen && (
              <div className="px-3 pb-3 space-y-2 border-t border-border pt-3">
                ...
                <p className="text-xs text-muted-foreground">
                  Condition types: overdue, sla_approaching, due_soon, pending_duration, dependency_count,
                  rejection_count, mission_status, agent_idle, label_match, priority_is, and, or
                </p>
                <p className="text-xs text-muted-foreground">
