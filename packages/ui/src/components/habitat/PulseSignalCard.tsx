...
export function PulseSignalCard({ pulse, missionId, reactionCounts, habitatId }: PulseSignalCardProps) {
    ...
    >
      <div className="p-3 space-y-2">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className={`text-[11px] font-medium text-[var(--on-surface)] ${pulse.isAuto ? 'text-[11px]' : 'text-xs'}`}>
            {pulse.fromType === 'system' ? 'System' : pulse.fromId.slice(0, 8)}
          </span>
        ...
        <div className="space-y-1">
          <p className={`font-semibold text-[var(--on-surface)] ${pulse.isAuto ? 'text-xs' : 'text-sm'}`}>
            {pulse.subject}
          </p>
          {pulse.body && (
            <p className={`text-[var(--on-surface-variant)] whitespace-pre-wrap ${pulse.isAuto ? 'text-[11px] leading-relaxed' : 'text-xs leading-relaxed'}`}>
