...
export function SprintPlanningPanel({ habitatId, onClose }: SprintPlanningPanelProps) {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setShowCreate(false);
      setForm({ name: '', goal: '', startDate: form.startDate, endDate: form.endDate, capacityMinutes: '' });
      refresh();
    } catch (err) {
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {activeSprint && (
          <div className="space-y-3">
            <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Active Sprint</p>
            <SprintCard
              sprint={activeSprint}
        ...
        {showCreate && (
          <div className="rounded-lg border border-border p-4 space-y-3">
            ...
            <div>
              <label className="mb-1 block text-xs text-muted-foreground" htmlFor="sprint-name">Name</label>
              <input
                id="sprint-name"
                type="text"
                value={form.name}
            ...
            <div>
              <label className="mb-1 block text-xs text-muted-foreground" htmlFor="sprint-goal">Goal (optional)</label>
              <input
                id="sprint-goal"
                type="text"
                value={form.goal}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="mb-1 block text-xs text-muted-foreground" htmlFor="sprint-start">Start Date</label>
                <input
                  id="sprint-start"
                  type="date"
              ...
              <div>
                <label className="mb-1 block text-xs text-muted-foreground" htmlFor="sprint-end">End Date</label>
                <input
                  id="sprint-end"
                  type="date"
                  value={form.endDate}
        ...
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              <SprintBadge sprintName={STATUS_LABELS[sprint.status] ?? sprint.status} sprintStatus={sprint.status} />
            </div>
            <p className="text-[10px] text-muted-foreground">
              {sprint.startDate} → {sprint.endDate} · {sprint.committedMissionIds.length} mission{sprint.committedMissionIds.length !== 1 ? 's' : ''}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-1 flex-shrink-0" onClick={e => e.stopPropagation()}>
          {sprint.status === 'planning' && (
            <Button size="sm" onClick={() => onStart(sprint.id)} loading={isActing} disabled={!!acting}>
              <Play className="h-3 w-3" /> Start
            </Button>
          )}
          {sprint.status === 'active' && (
            <Button size="sm" variant="success" onClick={() => onComplete(sprint.id)} loading={isActing} disabled={!!acting}>
              <CheckCircle className="h-3 w-3" /> Complete
            </Button>
          )}
          {(sprint.status === 'planning' || sprint.status === 'active') && (
            <Button size="sm" variant="ghost" onClick={() => onCancel(sprint.id)} disabled={!!acting}>
            ...
            {sprint.committedMissionIds.length === 0 ? (
              ...
              <div className="space-y-1">
                {sprint.committedMissionIds.map(mId => {
                  ...
                  return (
                    <div key={mId} className="flex items-center justify-between rounded px-2 py-1 bg-muted/50">
                      <span className="text-xs truncate">{feat?.title ?? truncateId(mId, 'FEAT')}</span>
                      {sprint.status === 'planning' && (
                        <button
                          type="button"
