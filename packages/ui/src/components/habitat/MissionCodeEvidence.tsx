...
export function MissionCodeEvidence({ missionId }: MissionCodeEvidenceProps) {
  ...
  const invalidateQueries = () => {
    queryClient.invalidateQueries({ queryKey: queryKeys.codeEvidence.mission(missionId) });
  ...
  const {
    ...
    queryKey: queryKeys.codeEvidence.mission(missionId),
    queryFn: () => api.codeEvidence.getMissionEvidence(missionId),
  });

  const linkMutation = useMutation({
    mutationFn: (url: string) =>
      api.codeEvidence.linkMissionCode(missionId, { pullRequestUrl: url }),
    onSuccess: () => {
  ...
  const notApplicableMutation = useMutation({
    mutationFn: (input: { reasonCode?: string; reasonNote?: string }) =>
      api.codeEvidence.markMissionNotApplicable(missionId, input),
    onSuccess: () => {
  ...
  const gapMutation = useMutation({
    mutationFn: (input: { reasonCode: string; reasonNote?: string }) =>
      api.codeEvidence.reportMissionGap(missionId, input),
    onSuccess: () => {
    ...
    return (
      <Card className="mb-4">
        <CardContent className="p-4 text-sm text-red-600 dark:text-red-400">
          Failed to load mission code evidence: {error.message}
        </CardContent>
      </Card>
    );
  }
