...
vi.mock('./AgentCard.js', () => ({
  AgentCard: ({ agent, currentTaskTitle, stats, expanded, onToggleExpand, onDeregister }: any) => (
    <div data-testid={`agent-card-${agent.id}`}>
      ...
      <button data-testid={`toggle-${agent.id}`} onClick={() => onToggleExpand(agent.id)}>Toggle</button>
      <button data-testid={`deregister-${agent.id}`} onClick={() => onDeregister(agent.id)}>Deregister</button>
    </div>
  ),
}));
...
describe('AgentPanel', () => {
  beforeEach(() => {
    ...
    mockAgentStats.mockReturnValue({
      data: { tasks: { completed: 5, failed: 1 }, cycleTime: { count: 1, averageMinutes: 30 }, quality: { rejectionRate: 0.1, currentStreak: 3 }, throughput: { last7d: 10 }, artifacts: { total: 2 } },
      isLoading: false,
    });
