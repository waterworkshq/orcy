...
vi.mock('../habitat/MissionHeader.js', () => ({
  formatStatus: (s: string) => s.replace(/_/g, ' ').replace(/\b\w/g, (c: string) => c.toUpperCase()),
}));
...
describe('FeatureTaskKanban', () => {
  ...
  it('renders task titles and priority badges', () => {
    const tasks = [
      makeTask({ id: 'task-abc123', title: 'My Important Task', priority: 'high', status: 'pending' }),
    ];
    render(<FeatureTaskKanban tasks={tasks} />);
  ...
  it('renders estimated time when present', () => {
    const tasks = [
      makeTask({ id: 't1', status: 'pending', estimatedMinutes: 45 }),
    ];
    render(<FeatureTaskKanban tasks={tasks} />);
  ...
  it('calls openModal on task card click', () => {
    const tasks = [
      makeTask({ id: 't1', title: 'Clickable Task', status: 'pending' }),
    ];
    render(<FeatureTaskKanban tasks={tasks} />);
  ...
  it('renders task with approved status in Review column', () => {
    const tasks = [
      makeTask({ id: 't1', title: 'Approved Task', status: 'approved' }),
    ];
    render(<FeatureTaskKanban tasks={tasks} />);
  ...
  it('renders task with rejected status in Review column', () => {
    const tasks = [
      makeTask({ id: 't1', title: 'Rejected Task', status: 'rejected' }),
    ];
    render(<FeatureTaskKanban tasks={tasks} />);
