...
export function SprintSelector({ habitatId, onOpenPlanning }: SprintSelectorProps) {
  ...
  if (!sprint) {
    return (
      <button
        ...
        aria-label="Open sprint management — no active sprint"
      >
  ...
  return (
    <button
      ...
      aria-label={`Open sprint management — ${sprint.name}, ${daysLeft} days left`}
    >
      <Timer
        className={`h-3.5 w-3.5 ${sprint.status === "active" ? "text-green-500" : "text-amber-500"}`}
      />
      <span className="font-medium text-on-surface truncate max-w-[120px]">{sprint.name}</span>
      {sprint.status === "active" && (
        <span
          className={`text-[10px] ${daysLeft <= 1 ? "text-red-500 font-bold" : daysLeft <= 3 ? "text-amber-500" : "text-on-surface-variant"}`}
        >
          {daysLeft}d left
        </span>
      )}
