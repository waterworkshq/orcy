...
import { useBoardEvents, useBoardAnomalies } from '../../lib/useHabitatData.js';`;
  ...
  it('changes filter and resets extra events', () => {
    ...
    expect(mockBoardEvents).toHaveBeenCalledWith('board-1', expect.objectContaining({
      action: 'claimed',
    }));
  });
  ...
  it('calls onClose when Close button clicked', () => {
    const onClose = vi.fn();
    renderWithQC(<ActivityPanel onClose={onClose} />);
  ...
  it('does not render Export button when no boardId', () => {
    ...
    mockStoreState.board = { id: 'board-1', name: 'Test Board' };
  });
});
