...
import { SIGNAL_LABELS, SIGNAL_COLORS, SIGNAL_TYPES } from "../../lib/signalConfig.js";
      ...
      {!collapsed && (
        <>
          <PulseFilterBar
            ...
            onToggleShowExperience={() => setShowExperience(!showExperience)}
            resultCount={total}
            onClearAll={clearAll}
          />
...
function HabitatPulseComposeDialog({
  habitatId,
  open,
  onClose,
}: {
    ...
    >
      ...
      >
        ...
        <p className="text-[11px] text-[var(--on-surface-variant)] mb-4">
          Broadcast a signal to the entire habitat
        </p>

        <form onSubmit={handleSubmit} className="space-y-3">
          <div className="flex flex-wrap gap-1.5">
            {(
              ["finding", "blocker", "warning", "directive", "context", "question"] as SignalType[]
            ).map((type) => (
              <button
                key={type}
                type="button"
