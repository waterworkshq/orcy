...
import { getAgentDisplayName, getActorDisplayName, formatTimestamp } from '../../lib/task-helpers.js';
...
const PRIORITY_CONFIG: Record<TaskPriority, { label: string; dotClass: string }> = {
  critical: { label: 'Critical', dotClass: 'bg-[var(--badge-critical)] shadow-[0_0_8px_rgba(139,69,69,0.35)]' },
  high: { label: 'High', dotClass: 'bg-[var(--badge-high)] shadow-[0_0_8px_rgba(139,69,83,0.35)]' },
  medium: { label: 'Medium', dotClass: 'bg-[var(--badge-medium)] shadow-[0_0_8px_rgba(139,113,69,0.35)]' },
...
const STATUS_DISPLAY: Record<TaskStatus, { label: string; className: string }> = {
  pending: { label: 'Pending', className: 'bg-surface-container-high/60 text-on-surface-variant border-outline-variant/30' },
  claimed: { label: 'Claimed', className: 'bg-[var(--badge-active-bg)] text-[var(--badge-active-text)] border-[var(--badge-active)]' },
  in_progress: { label: 'In Progress', className: 'bg-primary-container/30 text-primary border-primary-container/50' },
  submitted: { label: 'Submitted', className: 'bg-[var(--badge-review-bg)] text-[var(--badge-review-text)] border-[var(--badge-review)]' },
  approved: { label: 'Approved', className: 'bg-[var(--badge-done-bg)] text-[var(--badge-done-text)] border-[var(--badge-done)]' },
  rejected: { label: 'Rejected', className: 'bg-[var(--badge-blocked-bg)] text-[var(--badge-blocked-text)] border-[var(--badge-blocked)]' },
  done: { label: 'Done', className: 'bg-[var(--badge-done-bg)] text-[var(--badge-done-text)] border-[var(--badge-done)]' },
  failed: { label: 'Failed', className: 'bg-[var(--badge-blocked-bg)] text-[var(--badge-blocked-text)] border-[var(--badge-blocked)]' },
};

function CustomScrollbar({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
...
function StatusBadge({ status }: { status: TaskStatus }) {
  ...
  return (
    <div className={`inline-flex items-center gap-2 px-3 py-1 border rounded-sm ${config.className}`}>
...
function mapTaskEventToActivityEvent(event: TaskEvent, agents: Agent[]): ActivityEvent {
  ...
  const statusTransition = event.fromStatus && event.toStatus
...
export function TaskDetailModal() {
  ...
  const overlayClasses = visible
    ? 'opacity-100'
    : 'opacity-0';
  const modalClasses = visible
    ? 'opacity-100 scale-100'
    : 'opacity-0 scale-95';

  const assigneeName = task
    ? getAgentDisplayName(task.assignedAgentId, agents)
    : 'Unassigned';
          ...
          <>
            ...
            <div className="relative px-10 py-5 flex flex-wrap gap-10 bg-surface-container-low/30 border-y border-outline-variant/10">
              <div className="flex flex-col gap-2">
                <span className="text-[10px] font-label uppercase tracking-widest text-on-surface-variant">Assignee</span>
                <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col gap-2">
                <span className="text-[10px] font-label uppercase tracking-widest text-on-surface-variant">Priority</span>
                <PriorityDot priority={task.priority} />
              </div>

              <div className="flex flex-col gap-2">
                <span className="text-[10px] font-label uppercase tracking-widest text-on-surface-variant">Status</span>
                <StatusBadge status={task.status} />
              </div>

              <div className="flex flex-col gap-2">
                <span className="text-[10px] font-label uppercase tracking-widest text-on-surface-variant">Due Date</span>
                <div className="flex items-center gap-2">
            ...
            <div className="relative flex-1 grid grid-cols-12 overflow-hidden">
              <div className="col-span-7 max-md:col-span-12 p-6 md:p-10" data-testid="task-detail-left-column">
                <CustomScrollbar className="max-h-[calc(90vh-340px)] pr-1">
                  <div className="space-y-8 max-w-2xl">
                    <div className="space-y-4">
                      <h3 className="text-xs font-label uppercase tracking-widest text-on-surface-variant">Description</h3>
                      {task.description ? (
                        <MarkdownContent
                        ...
                        <div className="space-y-1">
                          {subtasks.map((subtask) => (
                            <div key={subtask.id} className="flex items-center gap-2 py-1.5">
                              ...
                              <span className={`text-sm ${subtask.completed ? 'line-through text-on-surface-variant' : 'text-on-surface'}`}>
                                {subtask.title}
                              </span>
                            </div>
                          ))}
              ...
              <div className="col-span-5 max-md:col-span-12 bg-surface-container-low/20 border-t md:border-t-0 md:border-l border-outline-variant/10 p-6 md:p-10" data-testid="task-detail-right-column">
                <CustomScrollbar className="max-h-[calc(90vh-340px)] pr-1">
                  <div className="space-y-8">
                    <section className="space-y-4">
                      <h3 className="text-xs font-label uppercase tracking-widest text-on-surface-variant">Activity Feed</h3>
                      <TaskActivityFeed events={activityEvents} />
                    </section>

                    <section className="space-y-4">
                      <h3 className="text-xs font-label uppercase tracking-widest text-on-surface-variant">Metadata</h3>
                      <div className="space-y-4 rounded-sm bg-surface-container-high/40 p-4">
                        ...
                        {task.estimatedMinutes && (
                          <div className="flex justify-between items-center gap-4">
                            ...
                            <span className="text-sm text-on-surface">{task.estimatedMinutes}m</span>
                          </div>
                        )}
                        ...
                        <div className="flex justify-between items-center gap-4">
                          ...
                          <span className="text-sm text-on-surface text-right">{formatTimestamp(task.updatedAt)}</span>
                        </div>
                      </div>

                      {task.result && (
                        <div className="p-4 bg-surface-container-high/80 rounded-sm">
                          <span className="text-[10px] font-label uppercase tracking-widest text-on-surface-variant">Result</span>
                          <p className="mt-1 text-sm text-on-surface/90 whitespace-pre-wrap">{task.result}</p>
                        </div>
                      )}
            ...
            <div className="relative p-6 md:p-8 bg-surface-container-high/50 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-t border-outline-variant/10">
              <div className="flex items-center gap-3">
                <button className="flex items-center gap-2 px-4 py-2 text-on-surface-variant hover:text-on-surface transition-colors text-sm font-medium" type="button">
