...
vi.mock("../components/habitat/DaemonSection.js", () => ({
...
describe("AgentsPage", () => {
  ...
  it("shows agent capabilities", () => {
    setListWithTasks(
      toListWithTasks([makeAgent({ id: "a1", capabilities: ["typescript", "react"] })]),
    );
    setStats("a1", makeStats({ agentId: "a1" }));
  ...
  it("shows relative time for last heartbeat", () => {
    ...
    setListWithTasks(toListWithTasks([makeAgent({ id: "a1", lastHeartbeat: fiveMinAgo })]));
    setStats("a1", makeStats({ agentId: "a1" }));
  ...
  it("shows just now for very recent heartbeat", () => {
    setListWithTasks(
      toListWithTasks([makeAgent({ id: "a1", lastHeartbeat: new Date().toISOString() })]),
    );
    setStats("a1", makeStats({ agentId: "a1" }));
  ...
  it("displays agent type and domain", () => {
    setListWithTasks(
      toListWithTasks([makeAgent({ id: "a1", type: "claude-code", domain: "frontend" })]),
    );
    setStats("a1", makeStats({ agentId: "a1" }));
  ...
  it("formats cycle time correctly with hours and minutes", () => {
    ...
    setStats(
      ...
      makeStats({
        agentId: "a1",
        cycleTime: { averageMinutes: 125, medianMinutes: 120, count: 5 },
      }),
    );
  ...
  it("formats cycle time with exact hours", () => {
    ...
    setStats(
      ...
      makeStats({
        agentId: "a1",
        cycleTime: { averageMinutes: 120, medianMinutes: 120, count: 3 },
      }),
    );
  ...
  it("shows hours ago for older heartbeat", () => {
    ...
    setListWithTasks(toListWithTasks([makeAgent({ id: "a1", lastHeartbeat: twoHoursAgo })]));
    setStats("a1", makeStats({ agentId: "a1" }));
  ...
  it("shows days ago for very old heartbeat", () => {
    ...
    setListWithTasks(toListWithTasks([makeAgent({ id: "a1", lastHeartbeat: threeDaysAgo })]));
    setStats("a1", makeStats({ agentId: "a1" }));
  ...
  it("renders agent card without capabilities", () => {
    setListWithTasks(toListWithTasks([makeAgent({ id: "a1", capabilities: [] })]));
    setStats("a1", makeStats({ agentId: "a1" }));
  ...
  it("displays current task title for working agent", () => {
    setListWithTasks(
      toListWithTasks([makeAgent({ id: "a1", name: "Agent 1", status: "working" })], {
  ...
  it("hides current task section when agent has no active task", () => {
    setListWithTasks(toListWithTasks([makeAgent({ id: "a1", name: "Agent 1", status: "idle" })]));
    setStats("a1", makeStats({ agentId: "a1" }));
