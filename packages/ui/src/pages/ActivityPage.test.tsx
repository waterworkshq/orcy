...
function makeEvent(
  overrides: Partial<EnrichedHabitatEvent> & { id: string },
...
function renderPage() {
  ...
  return render(
    <QueryClientProvider client={qc}>
      <MemoryRouter initialEntries={["/habitats/board-1/activity"]}>
        <Routes>
...
function setEventsResult(
  overrides: Partial<UseQueryResult<{ events: EnrichedHabitatEvent[]; total: number }>> = {},
) {
  mockEventsResult = {
    data: { events: [], total: 0 },
    isLoading: false,
...
describe("ActivityPage", () => {
  ...
  it("shows relative timestamps for events", () => {
    ...
    const events = [makeEvent({ id: "e1", timestamp: fiveMinAgo })];
  ...
  it("shows loading text on load more button while fetching", () => {
    const firstBatch = Array.from({ length: 50 }, (_, i) => makeEvent({ id: `e${i}` }));
    setEventsResult({ data: { events: firstBatch, total: 100 } });
    ...
    setEventsResult({
      isFetching: true,
      isLoading: false,
      data: { events: firstBatch, total: 100 },
    });
  ...
  it('shows "Human" label for human actors with no name', () => {
    const events = [makeEvent({ id: "e1", actorType: "human", actorName: null })];
    setEventsResult({ data: { events, total: 1 } });
  ...
  it('shows "System" label for system actors with no name', () => {
    const events = [makeEvent({ id: "e1", actorType: "system", actorName: null })];
    setEventsResult({ data: { events, total: 1 } });
