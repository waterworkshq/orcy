...
describe("task-helpers", () => {
  ...
  describe("getActorDisplayName", () => {
    ...
    it("returns Remote Or prefix for remote_orcy type", () => {
      const event = {
      ...
      const result = getActorDisplayName(event, agents);
      expect(result).toContain("Remote Or");
      expect(result).toContain("12345678");
    });

    it("returns Remote User prefix for remote_human type", () => {
      const event = {
        actorType: "remote_human",
        actorId: "abcdef01-2345-6789-abcd-ef0123456789",
        id: "e1",
        taskId: "t1",
        action: "updated",
        fromColumnId: null,
    ...
    it("returns Remote Pod prefix for remote_pod type", () => {
      const event = {
        actorType: "remote_pod",
        actorId: "aabbccdd-1234-5678-9abc-def012345678",
        id: "e1",
        taskId: "t1",
        action: "claimed",
        fromColumnId: null,
      ...
      const result = getActorDisplayName(event, agents);
      expect(result).toContain("Remote Pod");
