...
/** Registers the `orcy suggest` subcommands (suggest-next-task) on the given {@link Command}. */
export function registerSuggestCommands(program: any) {
  ...
  suggest
    .command("suggest-next-task")
    .description("Get AI-ranked task suggestions")
    .argument("<habitatId>", "Habitat UUID")
    .option("--limit <n>", "Max suggestions", "5")
    .action(
      withErrorHandling(async (habitatId: string, options: { limit: string }) => {
        const config = getOrcyConfig();
        const agentId = config.agentId;
        const result = await api.get<any>(
          `/api/agents/${agentId}/suggestions?habitatId=${encodeURIComponent(habitatId)}&limit=${options.limit}`,
        );
