...
function startCallbackServer(): Promise<{ port: number; callback: Promise<{ code: string; state: string }> }> {
...
function printIntegrationGuide(provider?: string): void {
  ...
  if (!selected || selected === 'all' || selected === 'linear') {
    ...
    console.log('  Optional override: ORCY_LINEAR_OAUTH_CLIENT_ID if you use your own Linear OAuth app.');
    console.log('');
  }

  if (!selected || selected === 'all' || selected === 'jira') {
    console.log('Jira Cloud setup');
    console.log('  Recommended: API token setup in the Orcy UI: Habitat Settings -> Integrations -> Jira Cloud.');
    console.log('  You need: Atlassian email, Atlassian API token, Jira site URL, and project key.');
    ...
    console.log('  Advanced OAuth: `orcy integrations connect <habitat-id> jira` only works when the API server has ORCY_JIRA_OAUTH_CLIENT_ID and ORCY_JIRA_OAUTH_CLIENT_SECRET configured.');
    console.log('  Secret handling: never commit Jira OAuth client secrets; self-hosted users must provide their own env vars.');
    console.log('');
  }
}
...
async function runOAuthConnect(habitatId: string, provider: 'jira' | 'linear'): Promise<void> {
  ...
  if (provider === 'jira') {
    console.log('Jira OAuth is an advanced self-hosted path. For most local installs, use Jira API token setup in the UI.');
    console.log('Required for OAuth: ORCY_JIRA_OAUTH_CLIENT_ID and ORCY_JIRA_OAUTH_CLIENT_SECRET on the API server.');
  }
  ...
  const result = await api.post<{ integration: Record<string, unknown> }>(
    completePath,
    {
      code: authCode,
...
export function registerIntegrationCommands(program: any) {
  const integration = program.command('integrations').description('External integration operations');

  integration.command('guide')
    .description('Show setup guidance for Jira and Linear integrations')
    .argument('[provider]', 'Provider: jira, linear, or all')
    .action(withErrorHandling(async (provider?: string) => {
  ...
  integration.command('connect')
    ...
    .action(withErrorHandling(async (habitatId: string, provider: string) => {
      if (provider !== 'jira' && provider !== 'linear') {
        throw new Error(`Provider '${provider}' is not supported for OAuth. Use 'jira' or 'linear'.`);
      }
      await runOAuthConnect(habitatId, provider);
    }));

  integration.command('sync')
    .description('Trigger manual sync for a connection')
    .argument('<connection-id>', 'Connection ID to sync')
    .action(withErrorHandling(async (connectionId: string) => {
      const result = await api.post<Record<string, unknown>>(`/api/integrations/${connectionId}/sync`);
  ...
  integration.command('list')
    ...
    .action(withErrorHandling(async (habitatId: string) => {
      const result = await api.get<Record<string, unknown>>(`/api/habitats/${habitatId}/integrations`);
      console.log(JSON.stringify(result, null, 2));
    }));

  integration.command('disconnect')
  ...
  integration.command('intake')
    .description('List intake candidates for a habitat')
    .argument('<habitat-id>', 'Habitat ID')
    .option('--status <status>', 'Filter by review status: new, needs_clarification, ready, promoted, ignored')
    .option('--provider <provider>', 'Filter by provider: jira, linear, github')
    .action(withErrorHandling(async (habitatId: string, options: { status?: string; provider?: string }) => {
      const params = new URLSearchParams();
      if (options.status) params.set('reviewStatus', options.status);
      if (options.provider) params.set('provider', options.provider);
      const qs = params.toString();
      const result = await api.get<Record<string, unknown>>(`/api/habitats/${habitatId}/intake-candidates${qs ? `?${qs}` : ''}`);
      console.log(JSON.stringify(result, null, 2));
    }));

  integration.command('promote')
    .description('Promote an intake candidate to a mission')
    .argument('<candidate-id>', 'Candidate ID to promote')
    .action(withErrorHandling(async (candidateId: string) => {
      const result = await api.post<Record<string, unknown>>(`/api/intake-candidates/${candidateId}/promote`);
      console.log(`Promoted to mission: ${(result as any).mission?.title ?? candidateId}`);
    }));

  integration.command('ignore')
