...
/** Registers the `orcy admin` subcommands (webhooks, templates, batch ops) on the given {@link Command}. */
export function registerAdminCommands(program: any) {
  const admin = program
    .command("admin")
    .description("Admin operations (webhooks, templates, batch)");
    ...
    .action(
      withErrorHandling(
        async (
          ...
          name: string,
          url: string,
          options: { events?: string; format: string },
        ) => {
  ...
  admin
    ...
    .action(
      withErrorHandling(async (webhookId: string) => {
  ...
  admin
    ...
    .action(
      withErrorHandling(async (habitatId: string, name: string, options: any) => {
  ...
  admin
    ...
    .action(
      withErrorHandling(async (templateId: string) => {
  ...
  admin
    .command("batch-set-priority")
    .description("Batch set task priority")
    .argument("<habitatId>", "Habitat UUID")
    .argument("<taskIds>", "Comma-separated task UUIDs")
    .argument("<priority>", "New priority: low, medium, high, critical")
    .action(
      withErrorHandling(async (habitatId: string, taskIds: string, priority: string) => {
        const ids = taskIds.split(",").map((s: string) => s.trim());
        const result = await api.post<any>(`/api/habitats/${habitatId}/tasks/batch`, {
          taskIds: ids,
          operation: "priority",
  ...
  admin
    .command("batch-delete-tasks")
    .description("Batch delete tasks")
    .argument("<habitatId>", "Habitat UUID")
    .argument("<taskIds>", "Comma-separated task UUIDs")
    .action(
      withErrorHandling(async (habitatId: string, taskIds: string) => {
        const ids = taskIds.split(",").map((s: string) => s.trim());
        const result = await api.post<any>(`/api/habitats/${habitatId}/tasks/batch`, {
          taskIds: ids,
          operation: "delete",
