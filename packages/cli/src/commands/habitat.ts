...
export function registerHabitatCommands(program: any) {
  ...
  habitat.command('update-settings')
    ...
    .action(withErrorHandling(async (habitatId: string, options: { name?: string; description?: string }) => {
  ...
  habitat.command('summary')
    ...
    .action(withErrorHandling(async (habitatId: string, options: { since: string; maxTasks: string; digest: boolean }) => {
