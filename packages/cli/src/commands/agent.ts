...
/** Registers the `orcy agent` subcommands (register, list, heartbeat, stats) on the given {@link Command}. */
export function registerAgentCommands(program: any) {
  ...
  agent
    ...
    .action(
      withErrorHandling(
        async (name: string, type: string, domain: string, options: { capabilities?: string }) => {
          const body: Record<string, any> = { name, type, domain };
          if (options.capabilities)
            body.capabilities = options.capabilities.split(",").map((s: string) => s.trim());
  ...
  agent
    ...
    .action(
      withErrorHandling(async (options: { status?: string; domain?: string }) => {
  ...
  agent
    ...
    .action(
      withErrorHandling(async (options: { taskId?: string; progress?: string }) => {
        const config = getOrcyConfig();
  ...
  agent
    ...
    .action(
      withErrorHandling(async () => {
        ...
        console.log(
          JSON.stringify({ agentId, ...agentData, stats: stats.stats ?? stats }, null, 2),
        );
      }),
    );
}
