...
function findOrcyApi(): ResolvedApi {
  const candidates: { path: string; isNodeScript: boolean }[] = [
    {
      path: path.join(ORCY_PATHS.home, "node_modules", "@orcy", "api", "dist", "index.js"),
      isNodeScript: true,
    },
...
function getSystemdPid(): number | null {
  try {
    const out = execSync("systemctl --user show orcy-api --property MainPID --value", {
      encoding: "utf-8",
    }).trim();
...
/** Registers the `orcy serve` subcommands (start, stop, status) for the local API server on the given {@link Command}. */
