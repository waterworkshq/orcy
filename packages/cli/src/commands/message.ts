...
/** Registers the `orcy message` subcommands (send, get-messages) on the given {@link Command}. */
export function registerMessageCommands(program: any) {
  ...
  msg
    ...
    .action(
      withErrorHandling(async (habitatId: string, subject: string, body: string, options: any) => {
        let toAgentId = options.toAgentId;
        if (!toAgentId && options.toAgentName) {
          const agents = await api.get<any>(
            `/api/agents?name=${encodeURIComponent(options.toAgentName)}`,
          );
