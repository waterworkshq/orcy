...
/** Registers the `orcy task` subcommands (list, create, claim, submit, quality gates) on the given {@link Command}. */
export function registerTaskCommands(program: any) {
  const task = program.command("task").description("Task operations");

  task
    .command("list-in-mission")
  ...
  task
    ...
    .option(
      "--domain <domain>",
      "Required agent domain (frontend, backend, devops, testing, fullstack)",
    )
    .option("--capabilities <caps>", "Comma-separated required capabilities")
    .option("--estimated-minutes <n>", "Estimated time in minutes")
    .action(
      withErrorHandling(async (missionId: string, title: string, options: any) => {
        const body: Record<string, any> = { title };
        if (options.description) body.description = options.description;
        if (options.priority) body.priority = options.priority;
        if (options.domain) body.requiredDomain = options.domain;
        if (options.capabilities)
          body.requiredCapabilities = options.capabilities.split(",").map((s: string) => s.trim());
    ...
    .action(
      withErrorHandling(async (taskId: string, options: any) => {
        ...
        if (options.estimatedMinutes !== undefined)
          body.estimatedMinutes = Number(options.estimatedMinutes);
  ...
  task
    ...
    .action(
      withErrorHandling(async (taskId: string, options: { limit: string; offset: string }) => {
        const normId = normalizeTaskId(taskId);
        const result = await api.get<any>(
          `/api/tasks/${normId}/events?limit=${options.limit}&offset=${options.offset}`,
        );
        console.log(JSON.stringify(result, null, 2));
      }),
    );
  ...
  task
    ...
    .action(
      withErrorHandling(async (taskId: string, options: { limit: string; offset: string }) => {
        const normId = normalizeTaskId(taskId);
        const result = await api.get<any>(
          `/api/tasks/${normId}/comments?limit=${options.limit}&offset=${options.offset}`,
        );
        console.log(JSON.stringify(result, null, 2));
      }),
    );
  ...
  task
    ...
    .action(
      withErrorHandling(async (taskId: string, content: string, options: { parentId?: string }) => {
        const normId = normalizeTaskId(taskId);
  ...
  task
    ...
    .action(
      withErrorHandling(async (taskId: string, dependsOnTaskId: string) => {
        const normId = normalizeTaskId(taskId);
        const result = await api.post<any>(`/api/tasks/${normId}/dependencies`, {
          dependsOnTaskId,
        });
        console.log(JSON.stringify(result, null, 2));
      }),
  ...
  task
    ...
    .action(
      withErrorHandling(async (taskId: string, dependencyTaskId: string) => {
        const normId = normalizeTaskId(taskId);
        const result = await api.delete<any>(
          `/api/tasks/${normId}/dependencies/${dependencyTaskId}`,
        );
        console.log(JSON.stringify(result, null, 2));
      }),
    );
  ...
  task
    .command("update-quality-checklist-item")
    ...
    .action(
      withErrorHandling(
        async (taskId: string, checklistId: string, itemId: string, options: any) => {
          const normId = normalizeTaskId(taskId);
          const body: Record<string, any> = {};
          ...
          const result = await api.put<any>(
            `/api/tasks/${normId}/quality-checklist/${checklistId}/items/${itemId}`,
            body,
          );
          console.log(JSON.stringify(result, null, 2));
        },
  ...
  task
    ...
    .action(
      withErrorHandling(async (taskId: string, title: string, options: { assigneeId?: string }) => {
        const normId = normalizeTaskId(taskId);
        const body: Record<string, any> = { title };
  ...
  task
    ...
    .action(
      withErrorHandling(async (taskId: string, subtaskId: string) => {
        const normId = normalizeTaskId(taskId);
