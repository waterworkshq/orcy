...
/** Registers the `orcy mission` subcommands (list, create, archive, context) on the given {@link Command}. */
export function registerMissionCommands(program: any) {
  ...
  mission
    ...
    .option(
      "--status <status>",
      "Filter by status (not_started, in_progress, review, done, failed)",
    )
    ...
    .action(
      withErrorHandling(
        async (
          habitatId: string,
          options: { status?: string; priority?: string; isArchived?: boolean; limit: string },
        ) => {
  ...
  mission
    ...
    .action(
      withErrorHandling(async (habitatId: string, title: string, options: any) => {
