  ...
  inputSchema: {
    ...
    properties: {
      ...
      description: {
        type: "string",
        description:
          'Mission description — the "brief" that provides context for all tasks within this mission',
      },
      acceptanceCriteria: {
        type: "string",
      ...
      slaMinutes: {
        type: "number",
        description:
          "Service-level agreement in minutes (triggers escalation if mission is not done by deadline = createdAt + slaMinutes)",
      },
      blocks: {
        type: "array",
        items: { type: "string" },
 ...
 * @requires CommentClient
 */
export async function missionListTasks(client: KanbanApiClient, args: { missionId: string }) {
...
export async function missionCreateTask(
  ...
  const publication = await client.publishTaskInMission(args.missionId, {
  ...
  if (publication.taskId === undefined) {
    throw new Error(`Task publication ${publication.attemptId} completed without a task id`);
  }
  ...
  switch (outcome.outcome) {
    case "created":
      if (outcome.recovering) {
        return {
          ...
          recovering: true,
          ...(outcome.recoveringState !== undefined && {
            recoveringState: outcome.recoveringState,
          }),
          message:
    ...
    case "vetoed":
      return {
        ...
        message:
          "Publication vetoed by governance — review the veto detail. An UNCHANGED retry with this attemptKey replays this terminal veto (no new side effect). To retry publication, use a NEW attemptKey.",
      };
  ...
  inputSchema: {
    ...
    properties: {
      ...
      requiredDomain: {
        type: "string",
        description:
          "Edited required-domain filter (frontend, backend, devops, testing, fullstack)",
      },
      requiredCapabilities: {
        type: "array",
        items: { type: "string" },
 ...
 * @requires CommentClient
 */
export async function missionGetContext(client: KanbanApiClient, args: { missionId: string }) {
...
export const HABITAT_DELETE_MISSION_TOOL: Tool = {
  ...
  description: "Delete a mission and all its tasks. This action is permanent and cannot be undone.",
  inputSchema: {
    type: "object",
    properties: {
      missionId: {
 ...
 * @requires CommentClient
 */
export async function habitatDeleteMission(client: KanbanApiClient, args: { missionId: string }) {
 ...
 * @requires CommentClient
 */
export async function missionArchive(client: KanbanApiClient, args: { missionId: string }) {
 ...
 * @requires CommentClient
 */
export async function missionUnarchive(client: KanbanApiClient, args: { missionId: string }) {
