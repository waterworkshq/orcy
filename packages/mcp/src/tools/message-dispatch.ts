...
/** MCP `Tool` registration schema for habitat point-to-point messaging (send, get-messages). */
export const MESSAGE_DISPATCH_TOOL: Tool = createDispatchTool({
  ...
  sharedParams: {
    ...
    messageType: {
      type: "string",
      enum: [...MESSAGE_TYPES],
      description: "Message type (action=send)",
    },
    priority: {
      type: "string",
      enum: [...MESSAGE_PRIORITIES],
      description: "Message priority (action=send)",
    },
    unreadOnly: {
      type: "boolean",
      description: "Only return unread messages (action=get-messages)",
    },
    limit: {
      type: "number",
      description: "Maximum number of messages to return (action=get-messages)",
    },
    offset: {
      type: "number",
      description: "Number of messages to skip for pagination (action=get-messages)",
    },
  },
});

/** Action-name → {@link Handler} map routing each message operation to its habitat client implementation. */
export const MESSAGE_ACTIONS: Record<string, Handler> = {
  send: habitatSendMessage,
  "get-messages": habitatGetMessages,
};

/** Top-level {@link ToolHandler} that routes incoming `orcy_habitat_message` MCP calls to the matching action. */
