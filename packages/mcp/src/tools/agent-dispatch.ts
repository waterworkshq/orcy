...
/** MCP `Tool` registration schema for habitat agent operations (register, list, heartbeat, get-stats). */
export const AGENT_DISPATCH_TOOL: Tool = createDispatchTool({
  ...
  sharedParams: {
    ...
    progress: {
      type: "string",
      description: "Brief description of current progress (action=heartbeat)",
    },
  },
});

/** Action-name → {@link Handler} map routing each agent operation to its habitat client implementation. */
...
/** Top-level {@link ToolHandler} that routes incoming `orcy_habitat_agent` MCP calls to the matching action. */
