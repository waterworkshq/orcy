...
const service = program.command("service").description("Manage the API systemd/launchd service");
...
function getDefaultServiceAction(platform: string) {
  ...
  return () => {
    console.log("Service management not available on this platform");
  };
}
...
service
  ...
  .action(() => {
    getDefaultServiceAction(getContext().platform)();
  });
...
export function resolveAction(rawArgs: string[]): Action {
  ...
  if (isWizardArg(first)) {
    // T4.3: validate EVERY arg, not just the first — `--yes --bogus` or
    // `--recover updtae` must error rather than silently installing while
    // ignoring the junk. The noninteractive wizard takes only recognized flags.
    for (const arg of rawArgs) {
      if (!isWizardArg(arg) && !BUILTIN_FLAGS.has(arg)) {
        return {
          kind: "error",
    ...
    return { kind: "noninteractive-wizard", opts: parseYesArgs(rawArgs) };
  }
...
// T4.1: a thrown error (e.g. the stale-journal hard-fail) must propagate a
// non-zero exit code so CI detects the failure — `console.error` alone exits 0.
main().catch((e) => {
  console.error(e);
  process.exitCode = 1;
});
