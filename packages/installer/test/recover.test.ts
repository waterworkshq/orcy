...
describe("T2.5 — both-files post-commit leftover is finalized, not recovered", () => {
  it("a journal whose done steps are already in the manifest is cleared without recovery (no throw even without --recover)", async () => {
    const { manifestPath } = await import("./helpers/setup.js");
    // Seed a manifest containing one created entry...
    const binPath = path.join(orcyHome(), "bin", "orcy");
    fs.mkdirSync(path.dirname(binPath), { recursive: true });
    fs.writeFileSync(binPath, "bin", "utf-8");
    fs.writeFileSync(
      manifestPath(),
      JSON.stringify({
        version: 1,
        ...
        components: ["cli"],
        files: [{ path: binPath, action: "created" }],
      }),
      "utf-8",
    );
    // ...and a journal with the SAME done step (the commit finished; only the
    // journal unlink was interrupted = the both-files window).
    seedJournal([{ path: binPath, action: "created" }]);

    // Without --recover this would normally THROW (stale journal). T2.5 finalizes
    // instead because the manifest already incorporates the journal's work.
    await expect(wizard(nonInteractive)).resolves.toBeUndefined();

    // The stale journal was discarded (finalized); the install completed.
    expect(fs.existsSync(journalPath()), "stale journal cleared").toBe(false);
  });
});
