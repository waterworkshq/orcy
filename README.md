...
- **Learning Loop** — a bounded, human-governed proposal loop that converts an allowlist of trustworthy Orcy history into immutable, cited findings. Sources include task/mission audit events, terminal automation/plugin/triage events, and k-anonymous experience aggregates (≥5 signals / ≥3 agents / ≥7-day windows). Human review with CAS-protected decisions (accept/reject/request-revision/withdraw); citation degradation blocks promotion. Accepted findings create at most one Habitat Wiki **draft** (never auto-publish). Agent reads via `orcy_learning` MCP tool requiring an active Task (actor-bound SQL predicate, no Habitat-wide fallback). Dormant by default (`ORCY_LEARNING_LOOP_ENABLED` + per-Habitat toggle). See [ADR-0044](docs/adr/0044-learning-loop-ledger-citations-and-lineage.md) and [ADR-0045](docs/adr/0045-learning-loop-authorization-and-privacy-propagation.md).
...
| Release | Theme |
|---------|-------|
| Next | **Learning Loop v1** — a bounded, human-governed proposal loop that converts trusted Orcy history into immutable, cited findings. Closed-allowlist sources (audit events, terminal runs, triage resolutions, k-anonymous experience aggregates). Human-only review with CAS decisions; accepted findings create Wiki drafts (never auto-publish). Agent task-scoped reads via `orcy_learning` MCP tool. Dormant by default (global env + per-Habitat toggle). Implementation complete; release pending. See [docs/ROADMAP.md](docs/ROADMAP.md). |
