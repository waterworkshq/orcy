...
| Release | Theme |
|---------|-------|
| v0.41.0 (implementation complete; release pending) | [Authoritative HTTP Route Assembly](docs/ROADMAP.md) — one staged assembly owns the production HTTP surface; policy-installed authentication on every route; declared (not raw-callback) plugin HTTP routes; Presence authenticated, the unverified provider-invite route removed, and unmatched CI webhooks fail closed under remote posture; Slack slash commands complete on their real form-encoded wire format (exact-byte verification unchanged). |
