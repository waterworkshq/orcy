...
- **Default 21** = first pass + three complete policy-driven fix rounds (3 + 3×6), or four no-policy rounds (3 + 4×4) with headroom — honoring the design intent of "three or four fix rounds" in both regimes. (The correction — raising the default from the original 12, which bought only the first pass plus two policy-driven rounds — was settled from the Ticket 2 final review; `DEFAULT_TASK_TRANSITION_CEILING` is the single source of the default.)
...
| Event-less mutation paths | Paths that mutate task state through the repo layer without emitting `task_events` (chat-command reject, plugin-context release, agent-service housekeeping releases, automation-driven release, retry scheduling) are charged nothing by construction — a pre-existing audit-trail completeness gap the meter inherits, tracked with exit criteria in `docs/deferred/roadmap/README.md` (deferred item TASK-EVT-1). Not a regression introduced by the budget. |
