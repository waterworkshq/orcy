...
| v0.22.1 | Channel Migration — extract Slack/Discord/in-app/webhook channels from `notificationDeliveryService.ts:52` switch into `plugins/channel-*/` reference plugins; fold `services/webhook-formatters/{discord,slack}.ts` into their respective channel plugin modules; collapse the switch to pure registry lookup. Optional buckle: ship the deferred `short-submission` + `rejection-loop` reference detector plugins if execution budget permits (they're single-file plugins that complete the seed's 3-detector reference set). Same theme: completing the notification-channel extraction proof matrix begun by the v0.22.0 `teams-channel` reference plugin. | Notification channel extraction surface (shipped in v0.22.0 per ADR-0017) — completes the proof by migrating the 4 in-tree channels to the new plugin surface |
| v0.22.2 | Detection Hardening — detector catch-up scan for missed events during server outage or detector enrollment latency (per ADR-0015); per-contribution `timeoutMs` watchdog for pre-hooks AND detectors (per ADR-0014 + ADR-0015 risk notes); `plugin_quarantines` persistent table for cross-restart quarantine state IF operator demand arises in dev-side validation phase (per ADR-0016). Same theme: hardening the plugin runtime for the production-readiness path beyond prerelease. | Detector execution seam (shipped in v0.22.0 per ADR-0015) + plugin auto-quarantine state (in-memory per v0.22.0 per ADR-0016) | 
