...
#### PATCH /triage/findings/:id — RETIRED

Returns `400 LEGACY_PATCH_RETIRED` with zero writes for EVERY legacy body shape (no-work `{status, bucket}`, link-only `{triageMissionId, expectedMissionVersion}`, unlink `{triageMissionId: null}`, mixed, terminal, and `targetRelease*` mutations alike — the body is not parsed). The legacy compatibility adapter (and the announced compatibility window) closed after v0.40.0; one deprecation telemetry line is logged per retired request. Old clients must upgrade: corrective work is created by `POST .../route`, and links are activated/terminalized exclusively through the lifecycle commands (`route`/`activate`/`resolve`/`wontfix`). Re-adding any PATCH shape is not the remediation.
