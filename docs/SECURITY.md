...
- All outbound fetch surfaces — plugin webhook calls, automation `call_webhook` actions, remote webhook registration and dispatch, the notification webhook channel, chat integrations, and outgoing webhooks — validate through the canonical `validateOutboundUrl` checker. Plugin webhook calls, automation actions, remote webhook dispatch, and the notification webhook channel additionally pin their fetch to the validated DNS resolution and fail closed on redirects
