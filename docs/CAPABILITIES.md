...
| **Release gates (Roadmap Activation)** | Missions can be gated behind a release type (`patch`/`minor`/`major`) or an exact version pin (`v0.25`). Gated missions block their tasks from claiming until a matching release ships; gates auto-resolve at read-time from the `releases` table. | [ARCHITECTURE.md](ARCHITECTURE.md) |
...
| **Plugin Ecosystem** | Safe, local-drop-in plugin platform. Manifest contract with 9 contribution kinds (`signalDetector`, `notificationChannel`, `lifecycleInterceptor`, `customMcpTool`, `customHttpRoute`, `webhookFormatter`, `automationCondition`, `automationAction`, `integrationProvider`) and a 9-capability whitelist gating plugin access. Per-habitat enrollment with detector allowlist. Audit Trail V2 integration. | [ARCHITECTURE.md](ARCHITECTURE.md) |
...
| **Typed workflow gates** | Six gate types: `on_complete` (upstream task completed), `on_approve` (upstream task approved), `on_signal` (matching pulse signal posted), `on_automation` (automation rule run completed — active since v0.20.1, with a habitat-scoped kill switch to toggle action execution), `on_manual` (admin unblock), `on_fail` (upstream task failed — spawns recovery). | [ARCHITECTURE.md](ARCHITECTURE.md) |
...
| **Privacy boundary** | Experience signals: aggregated-only in wiki UI/MCP (protect self-reporting honesty). System-internal consumers (triage scan) access individual signals via service layer. Findings: no privacy gate (intentional observations). | [ARCHITECTURE.md](ARCHITECTURE.md) |

## Triage (v0.23)

| Capability | What it does | Learn more |
|---|---|---|
| **Reactive Triage** | Periodic `signal_pattern_clustered` scan detects when 3+ implicit signals (experience/finding/detected) share the same normalized subject within a 7-day window. Creates a triage mission with an investigation task claimed by a daemon agent. Source-tagged analysis pulses + active-triage suppression prevent loops. | [ARCHITECTURE.md](ARCHITECTURE.md) |
| **Proactive Triage** | When a triage mission or finding resolves, the resolution (root cause, fix, kind) is recorded keyed by clusterKey. New clusters with matching keys surface the historical resolution as a suggested fix before creating new work. Agents query via `orcy_triage` MCP tool. | [ARCHITECTURE.md](ARCHITECTURE.md) |
| **Agent Quality Triggers** | Periodic `agent_quality_degraded` scan evaluates each agent's composite quality score against a configurable threshold with a sample-size gate. Fires informational automation actions (notify admin, create review mission) — does NOT control assignment, gates, or permissions. | [ARCHITECTURE.md](ARCHITECTURE.md) |
...
| **Restored lifecycle commands (ADR-0048)** | All production Finding lifecycle writes go through one command kernel: `POST /triage/findings/:id/route` (claim-bound agent routing into one gated/ungated Corrective Mission, canonical `correctiveMissionId`), `/activate` (manual activation of the EXISTING corrective Mission — clears only the release gate), `/resolve` + `/wontfix` (human-only terminal with full Resolution payload). Terminal rows are immutable; recurrence creates a new row with persisted lineage. `/promote` is retired (404). | [API.md](API.md) |
| **Terminal immutability + evidence protection** | `resolved`/`wontfix` never return to a non-terminal state (no route, no raw writer). Enforced invariants (migration `0068`, staged preflight-gated): partial-unique ACTIVE identity `(habitat_id, cluster_key, finding_kind)`, partial-unique Finding Resolution `(source, source_id)`, and RESTRICT FKs so referenced Pulse/Mission deletion cannot erase terminal evidence. | [DATABASE.md](DATABASE.md) |
| **Stable occurrence intake** | First-writer-frozen canonical occurrences (`triage_publication_occurrences`): versioned JCS/SHA-256 snapshot identity; the winner freezes the rendered payload and the complete prepared Mission/Task/workflow aggregate; conflicting workers publish exactly the persisted snapshot (template mutation after the win cannot reshape it). | [ARCHITECTURE.md](ARCHITECTURE.md) |
| **Release epochs + Automation handoff** | Each Release freezes one immutable activation epoch (cap, eligible groups, exact Finding ids); activation reconciles per-Mission under locked transactions and completed epochs never reopen. `release.shipped` hands off to immutable Automation rule revisions through a durable fenced inbox with proof-aware `attention_required` recovery — never to mutable live rules. `POST /triage/release-trigger` responses carry `incompleteProjections` when projections are still pending. | [API.md](API.md) |
...
| Capability | What it does | Learn more |
|---|---|---|
| **Semver-targeted deferrals (superseded)** | Deferral is now a release-gated Corrective Mission (ADR-0048 supersedes ADR-0029's activation semantics). The legacy `targetRelease`/`targetReleaseType` columns are read-only; mutations are rejected as superseded. | [CONTEXT.md](../CONTEXT.md) |
...
| **Epoch activation** | Matched deferred findings activate through the frozen Release epoch under the same kernel as manual activation (gate retained, `maxPromotionsPerRelease` hard Finding-count cap, all-or-none groups). Per-finding isolation means a mid-batch failure is counted as errored and the loop continues; deferred/oversized groups wait for a later Release or manual activation. | [API.md](API.md) |
