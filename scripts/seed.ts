...
import { createHabitat } from "../packages/api/src/services/habitatService.js";
...
async function seed() {
  ...
  const backlogColumn = columns.find(
    (c: { name: string }) => c.name === "Backlog" || c.name === "Todo",
  )!;
  ...
  const tasks = [
    {
      ...
      description:
        "Initialize pnpm workspaces with api, ui, and mcp packages. Configure TypeScript, ESLint, and build tooling.",
    ...
    },
    {
      ...
      description:
        "Build the core task lifecycle state machine with all transitions: pending→claimed→in_progress→submitted→approved→done. Include rejection handling.",
    ...
    },
    {
      ...
      description:
        "Implement transaction-based atomic claiming using BEGIN IMMEDIATE + SELECT FOR UPDATE. Ensure zero double-claims under concurrent load.",
      priority: "critical" as const,
    ...
    },
    {
      ...
      description:
        "Build the MCP stdio server with orcy_list_tasks, orcy_claim_task, orcy_update_task_status, orcy_submit_task, orcy_get_task_context, orcy_release_task, and orcy_heartbeat tools.",
      priority: "high" as const,
      labels: ["backend", "mcp"],
      requiredDomain: "backend",
    },
    {
      ...
      description:
        "Design SQLite schema for habitats, columns, missions, tasks, agents, task_events, and task_dependencies. Include all indexes and foreign keys.",
      priority: "high" as const,
      labels: ["backend", "database"],
      requiredDomain: "backend",
    },
    {
      ...
      description:
        "Add Server-Sent Events endpoint for real-time habitat updates. Publish task.created, task.moved, task.claimed, task.updated events.",
    ...
    },
    {
      ...
      description:
        "Create the React kanban board with drag-and-drop using dnd-kit. Columns, task cards, task detail panel, and agent status indicators.",
      priority: "high" as const,
      labels: ["frontend", "ui"],
      requiredDomain: "frontend",
    },
    {
      ...
      description:
        "Write vitest unit tests for: state machine transitions, atomic claim race conditions, invalid transition rejection, event audit logging.",
      priority: "medium" as const,
    ...
    },
    {
      ...
      description:
        "Write the SKILL.md template for Claude Code / Codex / OpenCode agents. Include startup sequence, task claiming rules, MCP tool usage examples.",
  ...
  createTask({
    ...
    description:
      "Build background job that runs every 60 seconds to detect and release tasks that have been idle for more than 30 minutes.",
    priority: "high",
    labels: ["backend", "core"],
    requiredDomain: "backend",
    createdBy: "system",
